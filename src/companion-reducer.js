...
// 提问工具：agent 询问用户时，桌宠播放 question 表情；用户回答后播放 answer 表情。
function isQuestionTool(name) {
  return String(name || '').toLowerCase() === 'ask_user_question'
}

// 从 tool/call 事件里提取提问文本（arguments 是模型产出的原始 JSON 字符串）。
function questionTextOf(event) {
  ...
  try {
    ...
    const texts = questions.map((question) => String(question?.question ?? '')).filter(Boolean)
...
// DSH 当前版本的 tool/result message 形如
// { source: { callId }, content: [{ type: 'tool-result', toolCallId, ... }], role, id }，
// toolCallId 嵌在 content[0]；兼容旧形状后取 callId。
...
export class CompanionReducer {
  ...
  handle(session, event) {
    ...
    switch (event.type) {
      ...
      case 'tool/call': {
        ...
        // STATE 在前、QUESTION 在后：helper 先按 STATE 更新底图与状态卡，
        // 再被 QUESTION 覆盖为提问表情和提问字幕，避免字幕被“干活中”文案顶掉。
  ...
  #update(record, state, payload) {
    // 同状态 + 同活动时复用旧文案，避免同一状态内变体文案随事件序号跳变（“字幕闪动”）
    const sameState = state === record.state
    const sameActivity = String(payload.activity ?? '') === String(record.payload?.activity ?? '')
    if (sameState && sameActivity && record.payload?.message) {
      payload = { ...payload, message: record.payload.message }
    }
    record.state = state
